...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<List<Station>>> getMyStationList(@RequestParam String userId) {
    ...
    public ResponseEntity<ApiResponse<String>> addMyStation(@RequestBody MyStationRequestDTO request) {
        ...
        if (isSuccess)
            return ResponseEntity.ok(new ApiResponse<>(null, "내 정류장에 성공적으로 추가되었습니다."));
        else
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiResponse<>(null, "내 정류장 추가에 실패했습니다."));
    }

    // 내 정류장 삭제
    @DeleteMapping("my-station/delete")
    public ResponseEntity<ApiResponse<String>> deleteMyStation(@RequestBody MyStationRequestDTO request) {
        ...
        if(isSuccess)
            return ResponseEntity.ok(new ApiResponse<>(null, "내 정류장이 성공적으로 삭제되었습니다."));
        else
            return ResponseEntity.badRequest().body(new ApiResponse<>(null, "내 정류장 삭제에 실패하였습니다."));
    }
